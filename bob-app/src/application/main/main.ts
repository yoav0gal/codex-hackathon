...
import type {
	MotionKeyCommand,
	MotionKeyGestureMode,
	MotionKeyResult,
} from "../../contracts/motionkey.js";
...
import {
	MotionKeyController,
	resolveMotionKeyPaths,
} from "./motionkey-controller.js";
...
config({
	path: path.join(app.getAppPath(), ".env.local"),
...
function registerIpc() {
	ipcMain.handle(
		...
		async (): Promise<IpcResult<{ value: string }>> =>
			protect(async () =>
	...
	ipcMain.handle(
		IPC.listSessions,
		async (): Promise<IpcResult<SessionSummary[]>> =>
			protect(() => sessions.list()),
	);
	ipcMain.handle(
		IPC.createSession,
		async (): Promise<IpcResult<ChatSession>> =>
			protect(() => sessions.create()),
	);
	ipcMain.handle(
		IPC.getSession,
		async (_event, id: unknown): Promise<IpcResult<ChatSession>> =>
			protect(() => sessions.get(requiredId(id))),
	);
	ipcMain.handle(
		IPC.appendMessage,
		async (
	...
	ipcMain.handle(
		IPC.deleteSession,
		async (_event, id: unknown): Promise<IpcResult<SessionSummary[]>> =>
			protect(() => sessions.delete(requiredId(id))),
	);
	ipcMain.handle(
		IPC.getWindowMode,
		async (): Promise<IpcResult<WindowMode>> => protect(async () => windowMode),
	);
	ipcMain.handle(
		IPC.setWindowMode,
		async (_event, mode: unknown): Promise<IpcResult<WindowMode>> =>
			protect(async () => setWindowMode(requiredWindowMode(mode))),
	);
	ipcMain.handle(
		IPC.minimizeWindow,
		...
		async (
			...
			protect(async () => {
				if (!(samples instanceof Float32Array) || samples.length > 32_768) {
					throw new Error(
						"Local wake detection received invalid microphone audio.",
					);
				}
				if (
					...
					throw new Error(
						"Local wake detection received an invalid sample rate.",
					);
				}
	...
	ipcMain.handle(
		IPC.controlCodex,
		async (_event, command: unknown): Promise<IpcResult<CodexCommandValue>> =>
			protect(() => codex.execute(requiredCodexCommand(command))),
	);
	ipcMain.handle(
		IPC.controlMotionKey,
		async (_event, command: unknown): Promise<IpcResult<MotionKeyResult>> =>
			protect(() => motionKey.execute(requiredMotionKeyCommand(command))),
	);
	ipcMain.handle(
		IPC.controlChrome,
		async (_event, command: unknown): Promise<IpcResult<ChromeResult>> =>
			protect(() => chrome.execute(requiredChromeCommand(command))),
	);
	ipcMain.handle(
		IPC.captureScreenshot,
		async (): Promise<IpcResult<ScreenshotCapture>> =>
			protect(captureCurrentScreen),
	);
}
...
async function captureCurrentScreen(): Promise<ScreenshotCapture> {
	if (process.platform === "darwin") {
		...
		if (permission === "denied" || permission === "restricted") {
			throw new Error(
				"Screen capture is blocked. Allow Bob under System Settings > Privacy & Security > Screen & System Audio Recording, then restart Bob.",
			);
		}
	}
	...
	const scale = Math.min(
		1,
		screenshotMaximumDimension / Math.max(physicalWidth, physicalHeight),
	);
	const thumbnailSize = {
	...
	if (!source || source.thumbnail.isEmpty()) {
		throw new Error(
			"Bob could not capture the current display. Check Screen & System Audio Recording permission and try again.",
		);
	}
...
function compressScreenshot(source: Electron.NativeImage) {
	...
	const maximumJpegBytes = Math.floor(
		((screenshotMaximumDataUrlLength - prefixLength) * 3) / 4,
	);
	...
	if (jpeg.length > maximumJpegBytes) {
		throw new Error(
			"Bob captured the screen, but could not make the image small enough for the live connection.",
		);
	}
...
function createCodexCapability() {
	...
	capability.onTaskUpdate((update) => {
		...
		if (window && !window.isDestroyed())
			window.webContents.send(IPC.codexTaskUpdate, update);
	});
	...
	void capability
		.connect()
		.catch((error) =>
			console.warn("Bob could not preconnect to Codex:", errorMessage(error)),
		);
	return capability;
}

function createMotionKeyController() {
	const { projectDir, python } = resolveMotionKeyPaths(
		app.getAppPath(),
		localEnvironment,
	);
...
function findCodexBinary() {
	if (localEnvironment.BOB_CODEX_BIN)
		return path.resolve(localEnvironment.BOB_CODEX_BIN);
...
function delegationsRoot() {
	return path.resolve(
		localEnvironment.BOB_DELEGATIONS_ROOT ||
			path.join(app.getPath("documents"), "Bob Delegations"),
	);
}
...
function requiredWindowMode(value: unknown): WindowMode {
	if (value !== "companion" && value !== "full")
		throw new Error("The window mode is invalid.");
	return value;
}
...
function requiredCodexCommand(value: unknown): CodexCommand {
	...
	if (command.type === "start") {
		return {
			...
			...(optionalShortText(command.workspace, "workspace", 4_096)
				? {
						workspace: optionalShortText(command.workspace, "workspace", 4_096),
					}
				: {}),
		};
	}
	if (command.type === "continue") {
		return {
			type: "continue",
			instruction: requiredShortText(
				command.instruction,
				"instruction",
				100_000,
			),
			effort: requiredEffort(command.effort),
			...(optionalShortText(command.thread, "thread", 1_000)
				? { thread: optionalShortText(command.thread, "thread", 1_000) }
				: {}),
		};
	}
	if (command.type === "monitor") {
		return {
			type: "monitor",
			thread: requiredShortText(command.thread, "thread", 1_000),
		};
	}
	if (command.type === "live") {
		const thread = optionalShortText(command.thread, "thread", 1_000);
		if (typeof command.enabled !== "boolean")
			throw new Error("The Codex Live setting is invalid.");
		return {
			type: "live",
...
function optionalShortText(
	...
	if (typeof value !== "string" || value.length > maximumLength)
		throw new Error(`The Codex ${field} is invalid.`);
...
function requiredMotionKeyCommand(value: unknown): MotionKeyCommand {
	...
	switch (command.type) {
		...
		case "start":
			return {
				type: "start",
				dryRun: motionKeyFlag(command.dryRun),
				preview: motionKeyFlag(command.preview),
			};
		case "stop":
			return { type: "stop" };
		case "status":
...
function requiredChromeCommand(value: unknown): ChromeCommand {
	if (!value || typeof value !== "object")
		throw new Error("The Chrome command is invalid.");
	const command = value as Record<string, unknown>;
	switch (command.type) {
		case "open": {
		...
		case "navigate":
			return { type: "navigate", url: requiredUrl(command.url) };
		case "newTab": {
			const url = optionalUrl(command.url);
		...
		case "activateTab":
			return { type: "activateTab", index: requiredTabIndex(command.index) };
		case "closeTab": {
...
function optionalUrl(value: unknown) {
	...
	if (typeof value !== "string" || !value.trim() || value.length > 8_192)
		throw new Error("The Chrome URL is invalid.");
	return value.trim();
}
...
function motionKeyMode(value: unknown): MotionKeyGestureMode {
	if (value !== "hold" && value !== "tap")
		throw new Error("The MotionKey mode is invalid.");
	return value;
}
...
function motionKeyFlag(value: unknown) {
	...
	if (typeof value !== "boolean")
		throw new Error("The MotionKey flag is invalid.");
	return value;
}
